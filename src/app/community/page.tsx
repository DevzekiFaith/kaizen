...
import { submitRating, subscribeToRatings, subscribeToCommunityStats } from '@/firebase/community';
        ...
        {stats && (
          <div className="border-4 border-slate-700 rounded-xl p-[1rem] mb-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-slate-600 text-[12px]">Average Inclusion Rating</p>
                <p className="text-2xl font-bold text-orange-500">{stats.averageInclusion.toFixed(1)}/10</p>
              </div>
              <div>
                <p className="text-slate-600 text-[12px]">Average Productivity Rating</p>
                <p className="text-2xl font-bold text-orange-500">{stats.averageProductivity.toFixed(1)}/10</p>
              </div>
              <div className="col-span-2">
                <p className="text-slate-600 text-[12px]">Total Ratings</p>
                <p className="text-2xl font-bold text-orange-500">{stats.totalRatings}</p>
              </div>
            </div>
          </div>
        )}
        ...
        {auth.currentUser && (
          <div className="border-4 border-slate-700 rounded-xl p-[1rem] mb-8">
            ...
            <div className="space-y-4">
              <div>
                <label htmlFor="inclusionRating" className="block text-sm font-medium text-slate-600 text-[12px]">Inclusion Rating</label>
                <input
                  id="inclusionRating"
                  type="range"
                  min="1"
                ...
                />
                <span className="text-sm text-slate-500">{inclusionRating}/10</span>
              </div>
              <div>
                <label htmlFor="productivityRating" className="block text-sm font-medium text-slate-600 text-[12px]">Productivity Rating</label>
                <input
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => setProductivityRating(Number(e.target.value))}
                  className="w-full accent-orange-500"
                />
                <span className="text-sm text-slate-500">{productivityRating}/10</span>
              </div>
              <div>
                <label htmlFor="comment" className="block text-sm font-medium text-slate-600 text-[12px]">Comment (Optional)</label>
                <textarea
                  id="comment"
                  value={comment}
            ...
            {ratings.map((rating, index) => (
              <div key={index} className="border-b border-slate-200 pb-4">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="font-semibold text-slate-600 text-[12px]">{rating.userName}</p>
                    <p className="text-sm text-slate-500 text-[10px]">
                      {new Date(rating.timestamp).toLocaleDateString()}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-slate-600 text-[12px]">Inclusion: {rating.inclusionRating}/10</p>
                    <p className="text-slate-600 text-[12px]">Productivity: {rating.productivityRating}/10</p>
                  </div>
                </div>
                {rating.comment && (
                  <p className="mt-2 text-slate-600 text-[12px]">{rating.comment}</p>
                )}
              </div>
            ))}
          </div>
